    ...
    <div class="white-box">
      ...
      <div class="row hidden p-b-20" id="add-inv">
        ...
        <div class="col-md-12 m-b-10">
          ...
          <button class="btn btn-success waves-effect waves-light m-r-10" id="asig-mbb" name="asig-mbb" type="button">
            Sim card
          </button>
          <button class="btn btn-success waves-effect waves-light m-r-10" id="asig-fiber" name="asig-fiber" type="button">
            Fibra
          </button>
        </div>
        ...
        <div class="row hidden" id="asig-fiber-content">
          <h3 class="box-title col-md-12">
            Buscar módem fibra
          </h3>
          <div class="col-md-8">
            <input class="typeahead form-control" id="list-art-fiber" placeholder="MSISDN" type="text">
          </div>
          <div class="col-md-4 text-left">
            <button class="btn btn-success waves-effect waves-light add-to-seller" type="button">
              Asignar inventario
      ...
      <div class="row hidden" id="info-seller">
        <h3 class="box-title">
          Inventario del instalador
        </h3>
        <div class="col-md-12 hidden" id="without-article">
          <div class="alert alert-danger">
            <p>
              Instalador sin inventario.
            </p>
          </div>
        </div>
...
<script type="text/javascript">
  $(function (){
    ...
    let invCoo = [],
        ...
        limitInvFiber = 0; {{-- Limite de inventario del vendedor mifi --}}
        ...
        @foreach($invFiber as $arti)
            invFiber.push("{{$arti->msisdn}}");
        @endforeach
        ...
        $('#list-art-fiber').typeahead(configList,{
                            name: 'articles',
                            source: substringMatcher(invFiber)
                          });
        ...
        $('#list-users').selectize({
            ...
            load: function(query, callback) {
                ...
                $.ajax({
                    ...
                    url: '{{ route('findRelationInstallers') }}',
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        q: query,
        ...
        $('#list-users').on('change', function(e){
            ...
            if(email && email != ''){
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    dataType: 'json',
                    url: "{{route('inventory.installers.findInveInstaller')}}",
                    data: { seller: email },
                    success: function(data){
                      //console.log(data);
                        if(!data.error){
                            ...
                            if(invCoo.length < 1 && invMbb < 1 && invMIFI.length < 1 && invFiber.length < 1){
                                if(data.isDismissal){
                                ...
                                if(!data.isSalesActive && !data.isDismissal && data.stockPF < data.limitInvFiber){
                                    $('#asig-fiber').show();

                                    if(isBtnAsigActive === false){
                                        isBtnAsigActive = 'asig-fiber';
                                    }
                                }
                                ...
                                if(data.isDismissal){
                                    ...
                                    $('#error-message-inv p').text('El instalador tiene ventas activas.');
                                    $('#error-message-inv').removeClass('hidden');
                                }else if(data.stockHbb >= data.limitInvHbb && data.stockMbb >= data.limitInvMbb && data.stockMbbstockPF >= data.limitInvFiber){
                                    $('#asig-hbb-content').addClass('hidden');
                                    $('#asig-mbb-content').addClass('hidden');

                                    $('#error-message-inv p').text('El instalador alcanzó el limite de inventario en módems y simcards.');
                                    $('#error-message-inv').removeClass('hidden');
                                } 
                            }
                            ...
                            limitInvFiber = data.limitInvFiber;
        ...
        $('.add-to-seller').on('click', function(e){
            ...
            if(num == ''){
              if( $('#list-art-fiber').val().trim() != "" ){
                num = $('#list-art-fiber').val().trim();
                type = 'FIBER';
              }else{
            ...
            if(num.length){
                ...
                $.ajax({
                    ...
                    url: "{{ route('inventory.installers.addStock') }}",
                    type: 'POST',
                    data:{
                        msisdn: num,
                        ...
                        if(!data.error){
                            if(data.articles){
                                ...
                                data.articles.forEach(function(ele){
                                    ...
                                    }else if(ele.artic_type == 'F'){
                                        //Quitando msisdn de la lista
                                        let index = invFiber.indexOf(ele.msisdn);
                                        if (index > -1) {
                                            invFiber.splice(index, 1);
                                        }
                                    }
                                });
                                ...
                                if(limitInvMbb <= $('#device-content blockquote.T').length && limitInv <= $('#device-content blockquote.H').length && limitInvMIFI <= $('#device-content blockquote.M').length && limitInvFiber <= $('#device-content blockquote.F').length){
                                    ...
                                    $('#asig-fiber-content').addClass('hidden');

                                    $('#error-message-inv p').text('El instalador alcanzó el limite de inventario en simcards y módems.');
                                    $('#error-message-inv').removeClass('hidden');
                                }else{
                                    if(limitInvMbb <= $('#device-content blockquote.T').length){
                                        ...
                                        $('#error-message-inv p').text('El instalador alcanzó el limite de inventario en simcards o teléfonos.');
                                        $('#error-message-inv').removeClass('hidden');
                                    }
                                    ...
                                    if(limitInv <= $('#device-content blockquote.H').length){
                                        ...
                                        $('#error-message-inv p').text('El instalador alcanzó el limite de inventario en módems.');
                                        $('#error-message-inv').removeClass('hidden');
                                    }
                                    ...
                                    if(limitInvMIFI <= $('#device-content blockquote.M').length){
                                        ...
                                        $('#error-message-inv p').text('El instalador alcanzó el limite de inventario en módems mifi.');
                                        $('#error-message-inv').removeClass('hidden');
                                    }

                                    if(limitInvFiber <= $('#device-content blockquote.F').length){
                                        $('#asig-fiber').hide();
                                        $('#asig-fiber-content').addClass('hidden');

                                        $('#error-message-inv p').text('El instalador alcanzó el limite de inventario en módems de Fibra.');
                                        $('#error-message-inv').removeClass('hidden');
                                    }
                                }
                                ...
                                if(invFiber.length < 1 && type == 'FIBER'){
                                    $('#asig-fiber-content').addClass('hidden');
                                    $('#error-message-inv p').text('No tienes disponible inventario de módems para asignar.');
                                    $('#error-message-inv').removeClass('hidden');
                                }
            ...
            $('.return-inv').on('click',function(e){
                ...
                if(num && seller){
                    ...
                    $.ajax({
                        ...
                        url: "{{ route('inventory.installers.removeStock') }}",
                        type: 'POST',
                        data:{
                            msisdn: num,
                            seller: seller,
                        ...
                        success: function(data){
                            if(!data.error){
                                if(data.msisdn){
                                    ...
                                    if(showAddInv && $('#device-content blockquote.F').length < limitInvFiber){
                                        $('#add-inv').removeClass('hidden');
                                        $('#asig-fiber').show();

                                        if($('#asig-fiber').hasClass('disabled') && $('#asig-fiber').is(':visible')){
                                            $('#asig-fiber-content').removeClass('hidden');
                                        }
                                    }
        ...
        $('#asig-hbb').on('click', function(e){
            ...
            $('#asig-fiber').removeClass('disabled');
            $('#asig-fiber').removeClass('btn-default');
            $('#asig-fiber').addClass('btn-success');
        ...
        $('#asig-fiber').on('click', function(e){
            $('.dn-t').each(function(){
                $(this).prop("checked", false);
            });
            ...
            $('#asig-fiber').addClass('disabled');
            $('#asig-fiber').addClass('btn-default');
            $('#asig-fiber').removeClass('btn-success');
