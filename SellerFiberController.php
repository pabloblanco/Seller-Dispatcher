      ...
      if (!empty($request->plan)) {
        ...
        if (in_array($bundle, $option)) {
          ...
          if ($bundle == 'N') {
            ...
            //Verificamos la categoria del los productos del bundle
            //
          ...
          $res = ['sucess' => false, 'msg' => "Los datos enviados tiene una incongruencia (568)"];
        }
      }
      return response()->json($res);
    }
    ...
    if ($request->isMethod('post') && $request->ajax()) {
      ...
      if (!empty($request->id) && !empty($request->type)) {
        ...
        if (!empty($data) && ($data->installer == session('user') || $data->installer_boss == session('user'))) {
          ...
          $response = ['error' => false, 'html' => $view, 'date_ins' => $data->date_instalation];
        }
      }
      return response()->json($response);
    }
  ...
  public function getQrForce(Request $request)
  {
    if ($request->isMethod('post') && $request->ajax()) {
      ...
      if ($request->isBundle == 'Y') {
        ...
        if ($confBundle['success']) {
          ...
          if (isset($confBundle->info_F->id)) {
            ...
            return response()->json(['success' => false, 'html' => '', 'htmlShare' => '', 'msg' => 'Se esta tratando de generar un contrato para un pack de tipo bundle pero no se pudo obtener el id del servicio de fibra']);
          }
        } else {
          return response()->json(['success' => false, 'html' => '', 'htmlShare' => '', 'msg' => 'Se esta tratando de generar un contrato para un pack de tipo bundle pero no se pudo obtener detalles del servicio de fibra']);
        }
      }
      ...
      if (!empty($urlPhotoF) && !empty($urlPhotoP)) {
        ...
        if (!$creDoc['success']) {
          return response()->json(['success' => false, 'html' => '', 'htmlShare' => '', 'msg' => $creDoc['msg']]);
        }
      } else {
        return response()->json(['success' => false, 'html' => '', 'htmlShare' => '', 'msg' => 'Los documentos de identificacion no se pudieron procesar']);
      }
      $contract_url = FiberPaymentForce::generateContract($request);
      if (!$contract_url['success']) {
        return response()->json(['success' => false, 'html' => '', 'htmlShare' => '', 'msg' => $contract_url['msg']]);
      } else {
      ...
      if (!$infoQr['success']) {
        return response()->json(['success' => false, 'html' => '', 'htmlShare' => '', 'msg' => $infoQr['msg']]);
      }
      ...
      if (!empty($info_client)) {
        ...
        $htmlShare = '<div class="container alert alert-danger">No se pudo precisar información del cliente </div>';
      }

      return response()->json(['success' => true, 'html' => $Qr_svg, 'htmlShare' => $htmlShare, 'msg' => 'OK']);
    }
    return redirect()->route('dashboard');
  }
 ...
 */
  ...
  {
    if ($request->isMethod('post') && $request->ajax()) {
      $response = ['success' => false, 'title' => "No se pudo verificar el QR", 'msg' => "Faltan datos para procesar la verificacion", 'icon' => "warning"];
      ...
      if ($data->is_payment_forcer == 'Y') {
        ...
        if (empty($data->payment_force_end)) {
          ...
          if (!$infoQr['success']) {
            ...
            return view('fiber.install_survey', compact('data', 'questions', 'answers', 'Qr_svg', 'htmlShare', 'htmlBundle'))->render();
          }
 ...
 */
    ...
    if (!empty($dataClient)) {
      ...
      if ($isBundle == 'N') {
        if (!$price) {
          ...
          if (empty($packprice)) {
            return ['success' => false, 'title' => "No se pudo cargar información asociada al cliente para la edición del correo de la pasarela de pago", 'msg' => "No se pudo obtener el detalle del paquete y el servicio de pago recurrente (CF3971)", 'icon' => "warning", 'code' => 'EMP_PAK'];
          }
        } else {
  ...
  public function getMailPayment(Request $request, $statusList = ['A', 'I'])
  {
    if ($request->isMethod('post') && $request->ajax()) {
      ...
      $response = ['success' => false, 'title' => "No se pudo cargar información asociada al cliente para la edición del correo", 'msg' => "Faltan datos para cambiar el correo para la pasarela de pago (CF4023)", 'icon' => "warning", 'code' => 'ERR_DAT'];
      ...
      if (!empty($dataCita)) {
        ...
        if (!empty($dataCita->pack_price_id)) {
          ...
          if (!empty($packprice)) {
            ...
            $response = ['success' => false, 'title' => "No se pudo cargar información asociada al cliente para la edición del correo", 'msg' => "No se pudo obtener el detalle del paquete y el servicio de pago recurrente (CF4075)", 'icon' => "warning", 'code' => 'EMP_PAK'];
          }
          ...
          if ($infPack['success']) {
            ...
            if ($infoAllBundle->general->containt_F == 'Y' && isset($infoAllBundle->info_F->id)) {
              ...
              $response = ['success' => false, 'title' => "No se pudo renovar el enlace de pago", 'msg' => "No se obtuvo una respuesta satisfactoria en la busqueda del pack de fibra dentro del combo (CF4064)", 'icon' => "warning", 'code' => 'EMP_PAK'];
            }
          } else {
            $response = ['success' => false, 'title' => "No se pudo renovar el enlace de pago", 'msg' => "Se presento un problema en obtener el detalle del bundle de susbcripcion ofrecido. (CF4067) Info: " . $infPack['data'], 'icon' => "warning", 'code' => 'EMP_BUN'];
          }
          ...
          if ($request->isBundle == 'Y') {
            ...
            if ($infPack['success']) {
              ...
              if ($infoAllBundle->general->containt_F == 'Y' && isset($infoAllBundle->info_F->id)) {
                ...
                $response = ['success' => false, 'title' => "No se pudo renovar el enlace de pago", 'msg' => "No se obtuvo una respuesta satisfactoria en la busqueda del pack de fibra dentro del combo (CF4123)", 'icon' => "warning", 'code' => 'EMP_PAK'];
              }
            } else {
              $response = ['success' => false, 'title' => "No se pudo renovar el enlace de pago", 'msg' => "Se presento un problema en obtener el detalle del bundle de susbcripcion configurado. (CF4122) Info: " . $infPack['data'], 'icon' => "warning", 'code' => 'EMP_BUN'];
            }
          }
          ...
          if ($servicio->for_subscription == 'Y') {
            ...
            if ($request->packNew) {
              //Significa que se esta cambiando el plan a uno con pago recurrente pero fallo el correo de netwey
              ...
              if ($request->isBundle == 'N') {
                ...
                if (!empty($serviceByPack)) {
                  ...
                  $response = ['success' => false, 'title' => "No se pudo cargar información asociada al cliente para la edición del correo", 'msg' => "No se encuentra el servicio asociado al nuevo pack en el sistema (CF4145)", 'icon' => "warning", 'code' => 'EMP_SER'];
                }
              }
            } else {
              $response = ['success' => false, 'title' => "No se pudo cargar información asociada al cliente para la edición del correo de la pasarela de pago", 'msg' => "No se encuentra el detalle de pago en el sistema (CF4152)", 'icon' => "warning", 'code' => 'EMP_PAY'];
            }
          }
        }

      } else {
        $response = ['success' => false, 'title' => "No se pudo cargar información asociada al cliente para la edición del correo de la pasarela de pago", 'msg' => "No se encuentra la cita en el sistema (CF4163)", 'icon' => "warning", 'code' => 'EMP_INS'];
      }
      return response()->json($response);
    }
    return redirect()->route('dashboard');
 ...
 */
  ...
  {
    if ($request->isMethod('post') && $request->ajax()) {
      Log::info('request 4359 ' . (String) json_encode($request->all()));
      ...
      if ($validate) {
        ...
        if (!empty($dataCita)) {
          ...
          Log::info('request->newpack 4373 ' . (String) $request->newpack);
          ...
          if ($request->is_bundle == 'Y') {
            ...
            if ($infPack['success']) {
              ...
              if ($infoAllBundle->general->containt_F == 'Y' && isset($infoAllBundle->info_F->id)) {
                ...
                $response = ['success' => false, 'title' => "No se pudo renovar el enlace de pago", 'msg' => "No se obtuvo una respuesta satisfactoria en la busqueda del pack de fibra dentro del combo (CF4383)", 'icon' => "warning", 'code' => 'EMP_PAK'];
              }
            } else {
              $response = ['success' => false, 'title' => "No se pudo renovar el enlace de pago", 'msg' => "Se presento un problema en obtener el detalle del bundle de susbcripcion configurado. (CF4386) Info: " . $infPack['data'], 'icon' => "warning", 'code' => 'EMP_BUN'];
            }
          } else {
          ...
          if (!empty($serviceByPack)) {
            ...
            if ($request->is_bundle == 'Y') {
              ...
              if ($infoAllBundle->general->containt_T == 'Y' && isset($infoAllBundle->info_T)) {
                ...
                if (!$updateChildren['success']) {
                  $response = ['success' => false, 'title' => "No se pudo actualizar el paquete de Telefonia del bundle para la instalación (CF4410)", 'msg' => $objUpd['msg'], 'icon' => "warning"];
                  return $response;
                }
              }
            } else {
            ...
            $response = ['success' => false, 'title' => "No se pudo actualizar el paquete para la instalación", 'msg' => "No se obtuvo una respuesta satisfactoria en la busqueda del servicio asociado al paquete (CF4424)", 'icon' => "warning"];
          }
        } else {
...
/**
 * [detailSupModal Retorna la informacion en el dasbord del instalador acerca del detalle de una cita de soporte]
 * @param  Request $request [description]
 * @return [type]           [description]
 */
  public function detailSupModal(Request $request)
  {
    if ($request->isMethod('post') && $request->ajax()) {
      $response = ['error' => true, 'message' => 'No se pudo cargar el detalle de la instalación.'];
