...
const videoSelect = document.querySelector('select#video_source');
...
function runSong(song) {
  var snapaudio = sound === true ? new Audio(soundsource[song]) : 'disable';
  if (snapaudio !== 'disable') {
    snapaudio.play();
  }
}

function runFlash(current) {
  if (flasheffect === true) {
    const canvasTag = $('#block_flash-' + current);
    canvasTag.removeClass().addClass('flasheffect').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function() {
      $(this).removeClass();
    });
  }
}
...
function btnTakePic(current, next) {
  ...
  for(var i = 0; i < videoTag[0].srcObject.getTracks().length; i++){
    videoTag[0].srcObject.getTracks()[i].stop();
  }
...
//Eliminacion de foto y preparar para otra
function btnDesPic(current, next, create=true) {
  //Eliminacion y nueva photo
  const videoTag = $('#video-' + current);
  resetPicFlow(videoTag, current);
  //initCam(videoTag);
  if(create){
    createCam(current, next);
...
//preparacion nuevo elemento de camara
function createCam(current, next) {
  videoElement = document.getElementById('video-' + current);
  if (current == "recibo") {
    // var b = document.getElementById("block_recibo");
    //  b.classList.remove('d-none');
    $('#block_recibo').attr('hidden', null);
  }
  getStream(current).then(getDevices).then(gotDevices);
}
//
//Procesos de vista de video
//
function getDevices() {
...
function gotStream(stream) {
  ...
  videoElement.setAttribute("height", heightRes);
}

function handleError(error) {
  console.error('Error: ', error.toString());
  alerta('Se presento un inconveniente con el uso de camara. ' + error.toString(), 6000);
}