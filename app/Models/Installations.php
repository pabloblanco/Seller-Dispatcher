...
class Installations extends Model
{
   ...
   *
   * @return App\Models\Installations
   */
  public static function getConnect($typeCon = false)
  {
    if ($typeCon) {
      $obj = new Installations;
  ...
  public static function getPendingDatesForInstaller($installer, $dateS = false, $dateE = false, $typeUser = "installer")
  {
    $dates = self::getConnect('R')
      ...
      ->where('islim_installations.status', 'A')
      ->whereIn('islim_installations.status_control', ['AB', 'AI']);

    if ($typeUser == 'installer') {
      //Asignadas a alguien para ser instalado el equipo
      $dates = $dates->where('islim_installations.installer', $installer);
    } elseif ($typeUser == 'ListinstallerBoss') {
      //Lista de instalaciones pendiente de los instaladores de un jefe dado
      $dates = $dates->where('islim_installations.installer_boss', $installer)
        ->whereNotNull('islim_installations.installer')
        ->whereNotIn('islim_installations.installer', [$installer]);
    } elseif ($typeUser == 'installerBoss') {
      //Lista de citas agendadas pero por asignar a un instalador
      $dates = $dates->where('islim_installations.installer_boss', $installer)
        ->whereNull('islim_installations.installer');
    }

    if ($dateS && $dateE) {

      $dates->where([
        ['islim_installations.date_instalation', '>=', $dateS],
        ['islim_installations.date_instalation', '<=', $dateE]]);
    } else {
      if ($dateS) {
        $dates->where('islim_installations.date_instalation', '>=', $dateS);
      } elseif ($dateE) {
        $dates->where('islim_installations.date_instalation', '<=', $dateE);
      }
    }
 ...
 */
  public static function getInstallationCaduce($installer, $typeUser = "installer")
  {
    $hoy = date("Y-m-d");
  ...
  public static function markAsInstalled($id, $msisdn, $prov = 'N')
  {
    try {
      if ($prov == 'Y') {
        self::getConnect('W')
  ...
  public static function getComponent($id)
  {
    $component = self::getConnect('R')
      ->select(
        ...
        'islim_installations.dataProvisioned'
      )
